layui.use(["table", "form", "layer", "element", "laydate"], function () {
    ...
    var API = {
        list: "/api/login-security-log/list",
        stats: "/api/login-security-log/stats",
        detail: "/api/login-security-log/detail/",
        hosts: "/api/login-security-log/hosts",
        export: "/api/login-security-log/export"
    };

    var LOGIN_TYPE_TEXT = {
        2: "2 - 交互登录", 3: "3 - 网络登录", 4: "4 - 批处理", 5: "5 - 服务",
        7: "7 - 解锁", 8: "8 - 网络明文", 9: "9 - 新凭据",
        10: "10 - 远程交互(RDP)", 11: "11 - 缓存交互"
    };

    var state = {
        loginResult: "",
        keyword: "",
        adv: {eventId: "", hostId: "", username: "", sourceIp: "", loginType: "", startTime: "", endTime: ""},
        sortField: "",
        sortOrder: ""
    };
    var hostOptions = [];
    ...
    function loginTypeText(t) {
        if (t == null || t === "") {
            return "-";
        }
        return LOGIN_TYPE_TEXT[t] || String(t);
    }

    function buildWhere() {
        var where = {};
        if (state.loginResult) {
            where.loginResult = state.loginResult;
        }
        if (state.keyword) {
        ...
        if (state.adv.sourceIp) {
            where.sourceIp = state.adv.sourceIp;
        }
        if (state.adv.loginType) {
            where.loginType = state.adv.loginType;
        }
    ...
    function renderTable() {
        table.render({
            ...
            cols: [[
                ...
                {field: "username", title: "用户名", minWidth: 180, templet: function (d) {
                    return escapeHtml(d.username) || "-";
                }},
                {field: "loginResult", title: "登录结果", width: 150, templet: "#resultTpl"},
                {field: "loginType", title: "登录类型", width: 150, templet: function (d) {
                    return escapeHtml(loginTypeText(d.loginType));
                }},
                {field: "sourceIp", title: "源IP", width: 140, templet: function (d) {
                    return escapeHtml(d.sourceIp) || "-";
                }},
    ...
    function loadStats() {
        AppRequest.request(API.stats, {method: "GET"}, {showErrorMessage: false})
            .then(function (res) {
                var d = res.data || {};
                document.getElementById("statSuccess").textContent = d.todaySuccess || 0;
                document.getElementById("statFail").textContent = d.todayFail || 0;
                document.getElementById("statLogout").textContent = d.todayLogout || 0;
                document.getElementById("statElevated").textContent = d.todayElevated || 0;
            })
        ...
        layer.open({
            ...
            success: function (layero) {
                ...
                form.val("advSearchForm", {
                    ...
                    sourceIp: state.adv.sourceIp,
                    loginType: state.adv.loginType
                });
                ...
                layero[0].querySelector("#advQueryButton").addEventListener("click", function () {
                    ...
                    state.adv.sourceIp = (field.sourceIp || "").trim();
                    state.adv.loginType = field.loginType || "";
                ...
                layero[0].querySelector("#advResetButton").addEventListener("click", function () {
                    state.adv = {eventId: "", hostId: "", username: "", sourceIp: "", loginType: "", startTime: "", endTime: ""};
                    form.val("advSearchForm", {eventId: "", hostId: "", username: "", sourceIp: "", loginType: ""});
                    rangeInput.value = "";
                    form.render(null, "advSearchForm");
                    renderActiveFilter();
                    reloadTable(true);
    ...
    function renderActiveFilter() {
        ...
        if (state.adv.sourceIp) {
            chips.push("源IP: " + state.adv.sourceIp);
        }
        if (state.adv.loginType) {
            chips.push("登录类型: " + loginTypeText(state.adv.loginType));
        }
        if (state.adv.startTime && state.adv.endTime) {
    ...
    function fillDetail(root, d) {
        var rows = [
            ...
            ["EventID", d.eventId],
            ["登录结果", d.loginResult],
            ["登录类型", loginTypeText(d.loginType)],
    ...
    async function exportLogs() {
        ...
        try {
            ...
            triggerDownload(blob, "login_security_logs_" + timestamp() + ".csv");
