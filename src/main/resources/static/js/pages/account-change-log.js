layui.use(["table", "form", "layer", "element", "laydate"], function () {
    ...
    var API = {
        list: "/api/account-change-log/list",
        stats: "/api/account-change-log/stats",
        detail: "/api/account-change-log/detail/",
        hosts: "/api/account-change-log/hosts",
        export: "/api/account-change-log/export"
    };

    var ACTION_TEXT = {
        create: "创建", enable: "启用", modify: "修改",
        change_password: "改密", add_to_group: "加入组", delete: "删除"
    };

    var state = {
        actionType: "",
        keyword: "",
        adv: {eventId: "", hostId: "", operatorUsername: "", targetUsername: "", startTime: "", endTime: ""},
        sortField: "",
        sortOrder: ""
    };
    var hostOptions = [];
    ...
    function actionText(a) {
        ...
        return ACTION_TEXT[String(a).toLowerCase()] || a;
    }

    function buildWhere() {
        var where = {};
        if (state.actionType) {
            where.actionType = state.actionType;
        }
        if (state.keyword) {
        ...
        if (state.adv.operatorUsername) {
            where.operatorUsername = state.adv.operatorUsername;
        }
        if (state.adv.targetUsername) {
            where.targetUsername = state.adv.targetUsername;
        }
    ...
    function renderTable() {
        table.render({
            ...
            cols: [[
                ...
                {field: "operatorUsername", title: "操作者", minWidth: 160, templet: function (d) {
                    return escapeHtml(d.operatorUsername) || "-";
                }},
                {field: "targetUsername", title: "目标用户", minWidth: 150, templet: function (d) {
                    return escapeHtml(d.targetUsername) || "-";
                }},
                {field: "actionType", title: "动作类型", width: 120, templet: "#actionTpl"},
                {field: "eventId", title: "EventID", width: 100, sort: true},
                {field: "details", title: "详情", minWidth: 240, templet: function (d) {
                    return '<span class="msg-cell" title="' + escapeHtml(d.details) + '">' + escapeHtml(d.details) + "</span>";
                }},
    ...
    function loadStats() {
        AppRequest.request(API.stats, {method: "GET"}, {showErrorMessage: false})
            .then(function (res) {
                var d = res.data || {};
                document.getElementById("statTotal").textContent = d.todayTotal || 0;
                document.getElementById("statCreate").textContent = d.todayCreate || 0;
                document.getElementById("statModify").textContent = d.todayModify || 0;
                document.getElementById("statDelete").textContent = d.todayDelete || 0;
            })
            .catch(function () { /* 忽略统计失败 */ });
    }
        ...
        layer.open({
            ...
            success: function (layero) {
                ...
                form.val("advSearchForm", {
                    ...
                    operatorUsername: state.adv.operatorUsername,
                    targetUsername: state.adv.targetUsername
                });
                ...
                layero[0].querySelector("#advQueryButton").addEventListener("click", function () {
                    ...
                    state.adv.operatorUsername = (field.operatorUsername || "").trim();
                    state.adv.targetUsername = (field.targetUsername || "").trim();
                ...
                layero[0].querySelector("#advResetButton").addEventListener("click", function () {
                    state.adv = {eventId: "", hostId: "", operatorUsername: "", targetUsername: "", startTime: "", endTime: ""};
                    form.val("advSearchForm", {eventId: "", hostId: "", operatorUsername: "", targetUsername: ""});
                    rangeInput.value = "";
                    form.render(null, "advSearchForm");
                    renderActiveFilter();
                    reloadTable(true);
    ...
    function renderActiveFilter() {
        ...
        if (state.adv.operatorUsername) {
            chips.push("操作者: " + state.adv.operatorUsername);
        }
        if (state.adv.targetUsername) {
            chips.push("目标用户: " + state.adv.targetUsername);
        }
        if (state.adv.startTime && state.adv.endTime) {
    ...
    function fillDetail(root, d) {
        var rows = [
            ["ID", d.id],
            ["EventID", d.eventId],
            ["动作类型", actionText(d.actionType)],
            ["事件时间", AppUtils.formatDateTime(d.eventTime)],
            ["主机", d.hostname ? d.hostname : ("#" + (d.hostId == null ? "-" : d.hostId))],
            ["主机IP", d.ipv4 || "-"],
            ["操作者", d.operatorUsername || "-"],
            ["目标用户", d.targetUsername || "-"],
            ["来源日志ID", d.sourceLogId],
        ...
        root.querySelector("#detailDetails").textContent = d.details || "-";
        root.querySelector("#detailMessage").textContent = d.logMessage || "（无关联原始日志）";
    ...
    async function exportLogs() {
        ...
        try {
            ...
            triggerDownload(blob, "account_change_logs_" + timestamp() + ".csv");
